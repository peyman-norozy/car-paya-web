...
const HeavyCarDevice = (props) => {
    ...
    const selectSearchOptionHandler = async (event) => {
            ...
            if (id === "brandOption") {
                ...
                const response = await getData(process.env.BASE_API + "/web" + API_PATHS.HEAVYCARMODELS + "/" + value)
                ...
                const response = await getData(process.env.BASE_API + "/web" + API_PATHS.HEAVYCARTIPS + "/" + value)
                if (response.status === 200) {
                    setNewYear([]);
                    setNewImage(
                ...
                const response = await getData(process.env.BASE_API + "/web" + API_PATHS.HEAVYCARYEARS + "/" + value)
                if (response.status === 200) {
    ...
    const myCarSubmitHandler = async (event) => {
            ...
            if (props.pageType === "edit" && Object.keys(newEditData).length > 0) {
                ...
                const response = await postData(process.env.BASE_API + "/user-panel" + API_PATHS.HEAVYCARS, fd, '"Content-Type": "application/json"')
    ...
    useEffect(() => {
        (async () => {
            const response = await getData(process.env.BASE_API + "/web" + API_PATHS.HEAVYCARBRANDS)
            if (response.status === 200) {
    ...
    useEffect(() => {
        if (props.pageType === "edit" && Object.keys(newEditData).length > 0) {
            let endpoints = [
                ...
                "/web" +
                API_PATHS.HEAVYCARTIPS +
                "/" +
                newEditData.car_model_id,
                process.env.BASE_API +
                "/web" +
                API_PATHS.HEAVYCARMODELS +
                "/" +
                newEditData.car_brand_id,
                process.env.BASE_API +
                "/web" +
                API_PATHS.HEAVYCARYEARS +
                "/" +
    ...
    return (
        <form className="flex-1 px-[40px] py-[32px] rounded-[10px] shadow-[0_0_6px_0_rgba(180,180,180,0.3)] flex flex-col gap-[35px]"
            ...
            <h1 className={"text-[#354597]"}>ماشین سنگین من</h1>
            ...
            >
                <SelectCarModal modalPosition={true} modalName={"heavyCarModal"} setModalState={setModalState}/>
            </div>}
        </form>
    );
};

export default HeavyCarDevice;