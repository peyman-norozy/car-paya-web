  ...
  return (
    <div className={"absolute right-0.5 h-full"}>
      ...
      >
        <ul
          className={
            "grid grid-cols-3 size411:grid-cols-4 sm:grid-cols-5 gap-2 px-4 py-4"
          }
        >
          {fakeData.map((item, index) => (
            <li
              key={index}
              className={`flex flex-col items-center cursor-pointer gap-2 ${cityId === item.id ? "bg-stone-200" : ""} py-1 rounded-8`}
