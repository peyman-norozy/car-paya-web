...
const VerificationSecondStep = (props) => {
  ...
  const continueSecondStepHandler = () => {
    axios.get(process.env.BASE_API + '/web/expert/reservation?step=step-3').then(res => {
      console.log(res)
  ...
  return (
      <div className={'flex items-start justify-between pt-[28px] w-[95%] size1190:w-[90%] m-auto'}>
        <div className={'w-full lg:w-[50%] flex flex-col'}>
          <div className={'flex items-center gap-2 size752:gap-[16px] text-BLUE_600 w-full mb-4'}>
            ...
            <p className={'p-[6px] text-14 size752:text-16 w-full border-b border-BLUE_600'}>چه زمانی اماده دریافت
              کارشناس هستید؟</p>
          </div>
          <div className={'flex flex-col gap-[2rem]'}>
            {data.map((item, index) => <ReserveTimeVerification data={item} timeIsSelected={timeIsSelected}
                                                                setTimeIsSelected={setTimeIsSelected}
                                                                setOptionIsOpen={setOptionIsOpen}
                                                                optionIsOpen={optionIsOpen} key={index}/>)}

          </div>
          <button
              className={'bg-BLUE_700 self-end flex items-center gap-2 mt-4 size690:mt-3 w-fit text-12 size690:text-[16px] p-[8px] text-white rounded-[4px]'}>
            <p>تایید و ادامه</p>
            <i className={'cc-left text-[20px]'}/>
          </button>
        </div>
        <Image src={'assets/images/reserveTimePic.svg'} alt={''} width={544} height={544}
