...
const MotorDevice = (props) => {
    ...
    const selectSearchOptionHandler = async (event) => {
            ...
            if (id === "brandOption") {
                ...
                const response = await getData(process.env.BASE_API + "/web" + API_PATHS.MOTORMODELS + "/" + value)
                ...
                const response = await getData(process.env.BASE_API + "/web" + API_PATHS.MOTORTIPS + "/" + value)
                if (response.status === 200) {
                    setNewYear([]);
                    setNewImage(
    ...
    useEffect(() => {
        (async () => {
            const response = await getData(process.env.BASE_API + "/web" + API_PATHS.MOTORBRANDS)
            if (response.status === 200) {
