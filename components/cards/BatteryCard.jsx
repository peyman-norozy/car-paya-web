  ...
  return (
    <div className="w-full text-14 size1160:text-16 rounded-10 shadow-[0_0_6px_0_rgba(177,177,177,1)]  p-[0.5rem] size1228:p-[1.5rem] pb-[1.5rem]  flex flex-col size671:flex-row gap-[1rem] size671:gap-0 items-center justify-between">
      ...
      <div className="w-full size671:w-auto ">
        <h1 className="text-[1.25rem] text-center size671:text-start font-bold text-[#303030]">{data.name}</h1>
        ...
        <ul className="flex flex-col gap-[0.5rem]">
          ...
          <li className="text-text_gray flex gap-[0.5rem]">
            <Image src={star} alt="" width={20} height={20} className="mb-[0.35rem] self-start mt-[0.25rem]"/>
      ...
      <Button on_click={() => setBatteryIsSelected(true)} class_name="bg-RED_500 rounded-[8px] text-white py-[0.5rem] px-[1rem] size1314:py-[1rem] size1314:px-[2rem] self-start size671:self-end hover:bg-RED_600">
        اضافه به سبد خرید
      </Button>
    </div>
  );
